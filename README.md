### pkaction Rules Generator - BUILT using Junie GPT 5.5

### Written by a human

The .rules file generated by this script should work on any linux using pkexec for privilege escalation. Tested on MX 23.

The `pkaction_compact{date}.rules` is the easiest one to use.

To use, created a group called "superuser" and add yourself to that group. Then place the .rules file in the /etc/polkit-1/rules.d/ directory and turn off the privilege escalation passwords by
uncommenting the entries. 

The easiest way to uncomment entries is to use geany, set for document format "javascript source", and use the Format-Uncomment command.
